...
// New creates a new controller that syncs the configmap containing kubelet CA(s)
// into operand namespace.
...
func (r *reconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	reqLogger.Info("reconciling kubelet CA configmap")
	...
	if _, _, err := r.ensureKubeletCAConfigMap(ctx); err != nil {
		return reconcile.Result{}, fmt.Errorf("failed to ensure the kubelet CA configmap: %w", err)
	}

	reqLogger.Info("kubelet CA configmap is reconciled")
