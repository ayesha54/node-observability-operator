...
func (r *MachineConfigReconciler) ensureReqNodeLabelExists(ctx context.Context) (int, error) {
	...
	for i, node := range nodeList.Items {
		...
		if err := r.Impl.ClientPatch(ctx, r.Client, &nodeList.Items[i], client.RawPatch(types.JSONPatchType, patch)); err != nil {
			return updNodeCount, err
		}
...
func (r *MachineConfigReconciler) revertNodeLabeling(ctx context.Context) error {
	...
	for name, label := range r.Node.PrevReconcileUpd {
		...
		node := &corev1.Node{}
		if err := r.Impl.ClientGet(ctx, r.Client, types.NamespacedName{Name: name}, node); err != nil {
		...
		if err := r.Impl.ClientPatch(ctx, r.Client, node, client.RawPatch(types.JSONPatchType, patch)); err != nil {
			return err
		}
...
func (r *MachineConfigReconciler) listNodes(ctx context.Context, nodeList *corev1.NodeList, matchLabels map[string]string) error {
	...
	if err := r.Impl.ClientList(ctx, r.Client, nodeList, listOpts...); err != nil {
		return err
	}
